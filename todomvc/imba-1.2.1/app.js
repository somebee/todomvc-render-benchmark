...
var App = _T.defineTag('App', function(tag){
	...
	tag.prototype.list = function (items){
		var __ = (this.__._list = this.__._list || {}), id_;
		let res = [];
		for (let i = 0, ary = iter$(items), len = ary.length, todo; i < len; i++) {
			todo = ary[i];
			res.push((__[(id_ = todo.id)] = __[id_] || Todo.build(this)).setData(todo).end());
		};
		return res;
	};
		...
		return this.setChildren([
			...
			(all.length > 0) ? (
				(__.B = __.B || _T.SECTION(self).flag('main')).setContent([
					(self._toggler || _T.INPUT(self).ref_('toggler',self).flag('toggle-all').setType('checkbox').setHandler('change','toggleAll',self)).end(),
					(__.BA = __.BA || _T.UL(self).flag('todo-list')).setContent(self.list(items),3).end()
				],2).end()
			) : void(0),
