...
var App = _T.defineTag('App', function(tag){
	...
	tag.prototype.list = function (items){
		var $ = (this.$._list = this.$._list || {}), id_;
		let res = [];
		for (let i = 0, ary = iter$(items), len = ary.length, todo; i < len; i++) {
			todo = ary[i];
			res.push(($[(id_ = todo.id)]=$[id_] || Todo.build(this)).setData(todo).end());
		};
		return res;
	};
		...
		return this.setChildren([
			...
			(all.length > 0) ? (
				($.D=$.D || _T.$('section',self).flag('main')).setContent([
					...
					($.E=$.E || _T.$('ul',self).flag('todo-list')).setContent(self.list(items),3).end()
				//	for todo,i in items
				//		<Todo[todo]>
				],2).end()
			) : void(0),
			...
			(all.length > 0) ? (
				($.F=$.F || _T.$('footer',self).flag('footer')).setContent([
					($.G=$.G || _T.$('span',self).flag('todo-count')).setContent([
						($.H=$.H || _T.$('strong',self)).setText("" + (active.length) + " ").end(),
						(active.length == 1) ? 'item left' : 'items left'
					],1).end(),
					($.I=$.I || _T.$('ul',self).flag('filters')).setContent([
						($.J=$.J || _T.$('li',self)).setContent(($.K=$.K || _T.$('a',self).setHref('#/').setText('All')).flag('selected',(items == all)).end(),2).end(),
						($.L=$.L || _T.$('li',self)).setContent(($.M=$.M || _T.$('a',self).setHref('#/active').setText('Active')).flag('selected',(items == active)).end(),2).end(),
						($.N=$.N || _T.$('li',self)).setContent(($.O=$.O || _T.$('a',self).setHref('#/completed').setText('Completed')).flag('selected',(items == done)).end(),2).end()
					],2).end(),
					(done.length > 0) ? (
						($.P=$.P || _T.$('button',self).flag('clear-completed').on('tap','clearCompleted',0).setText('Clear completed')).end()
					) : void(0)
				],1).end()
			) : void(0)
		],1).synced();
...
Imba.Tag.prototype.set = function (key,value){
