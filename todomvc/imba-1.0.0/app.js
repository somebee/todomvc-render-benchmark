...
/***/ function(module, exports, __webpack_require__) {
	...
	var Store = __webpack_require__(16).Store;
	Todos = new Store('imba-todos');

	var ENTER_KEY = 13;

	_T.defineTag('app', function(tag){
		...
		tag.prototype.list = function (items){
			...
			for (var i = 0, ary = Imba.iterable(items), len = ary.length, todo, res = []; i < len; i++) {
				todo = ary[i];
				res.push((__[todo.id] = __[todo.id] || _T.TODO(this)).setData(todo).end());
			};
			return res;
		};
		...
		tag.prototype.render = function (){
			...
			for (var i = 0, ary = Imba.iterable(all), len = ary.length, todo; i < len; i++) {
				todo = ary[i];
			...
			return this.setChildren([
				(__.A = __.A || _T.HEADER(this).flag('header')).setContent([
					(__.AA = __.AA || _T.H1(this)).setContent(("todos " + (API.RENDERCOUNT)),3).end(),
					(__.AB = __.AB || _T.INPUT(self).flag('new-todo').setType('text').setPlaceholder('What needs to be done?').setAutofocus(true)).end()
				],2).end(),
				
				(all.length > 0) ? (
					(__.B = __.B || _T.SECTION(self).flag('main')).setContent([
						(__.BA = __.BA || _T.INPUT(self).flag('toggle-all').setType('checkbox').setHandler('change','toggleAll',self)).end(),
						(__.BB = __.BB || _T.UL(self).flag('todo-list')).setContent(self.list(items),3).end()
					],2).end()
				) : void(0),
				
				(all.length > 0) ? (
					(__.C = __.C || _T.FOOTER(self).flag('footer')).setContent([
						(__.CA = __.CA || _T.SPAN(self).flag('todo-count')).setContent([
							(__.CAA = __.CAA || _T.STRONG(self)).setContent(("" + (active.length) + " "),3).end(),
							(active.length == 1) ? 'item left' : 'items left'
						],1).end(),
						(__.CB = __.CB || _T.UL(self).flag('filters')).setContent([
							(__.CBA = __.CBA || _T.LI(self)).setContent((__.CBAA = __.CBAA || _T.A(self).setHref('#/').setText('All')).flag('selected',(items == all)).end(),2).end(),
							(__.CBB = __.CBB || _T.LI(self)).setContent((__.CBBA = __.CBBA || _T.A(self).setHref('#/active').setText('Active')).flag('selected',(items == active)).end(),2).end(),
							(__.CBC = __.CBC || _T.LI(self)).setContent((__.CBCA = __.CBCA || _T.A(self).setHref('#/completed').setText('Completed')).flag('selected',(items == done)).end(),2).end()
						],2).end(),
						(done.length > 0) ? (
							(__.CC = __.CC || _T.BUTTON(self).flag('clear-completed').setHandler('tap','clearCompleted',self).setText('Clear completed')).end()
						) : void(0)
					],1).end()
				) : void(0)
			],1).synced();
	...
	var app = _T.APP(this).setId('app').end();
...
/***/ function(module, exports) {
	...
	Imba.TagManagerClass.prototype.tryMount = function (){
		...
		for (var i = 0, ary = Imba.iterable(items), len = ary.length, el; i < len; i++) {
			el = ary[i];
			if (el && el._tag) {
...
/***/ function(module, exports) {
	...
	_T.defineTag('todo', 'li', function(tag){
		...
		tag.prototype.render = function (){
			...
			return this.flag('completed',(todo.completed)).setChildren([
				(__.A = __.A || _T.DIV(this).flag('view')).setContent([
					(__.AA = __.AA || _T.LABEL(this).setHandler('dblclick','edit',this)).setContent(("" + (todo.title)),3).end(),
					(self._toggle || _T.INPUT(self).ref_('toggle',self).setType('checkbox').setHandler('change','toggle',self)).setChecked((todo.completed)).end(),
					(__.AB = __.AB || _T.BUTTON(self).flag('destroy').setHandler('tap','drop',self)).end()
				],2).end(),
...
/***/ function(module, exports) {
	...
	Store.prototype.inform = function (){
		if (API.AUTORENDER) {
			for (var i = 0, ary = Imba.iterable(this._listeners), len = ary.length; i < len; i++) {
				ary[i](this);
			};
		};
	...
	Store.prototype.toggleAll = function (state){
		for (var i = 0, ary = Imba.iterable(this.items()), len = ary.length; i < len; i++) {
